"""Los Angeles Type Directors Club competition crawler."""
...
class LaTdcCrawler(BaseCrawler):
    """Crawler for LA TDC (https://latdc.org)."""

    BASE_URL = "https://latdc.org"

    def __init__(self):
        super().__init__("LA Type Directors Club", SourceType.COMPETITION)

    async def fetch_articles(self, **kwargs) -> list[Article]:
        """Fetch competition listings from LA TDC."""
        articles = []
        headers = {
        ...
        async with httpx.AsyncClient(timeout=30.0, headers=headers) as client:
            try:
                ...
                entries = soup.select("div.project-item, article.entry, div.latdc-project")
