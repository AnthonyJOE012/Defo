...
class MuseDesignCrawler(BaseCrawler):
    """Crawler for Muse Design Awards (https://musedesignawards.com)."""

    BASE_URL = "https://musedesignawards.com"

    def __init__(self):
        super().__init__("Muse Design Awards", SourceType.COMPETITION)

    async def fetch_articles(self, **kwargs) -> list[Article]:
        """Fetch competition listings from Muse Design Awards."""
        articles = []
        headers = {
        ...
        async with httpx.AsyncClient(timeout=30.0, headers=headers) as client:
            try:
                ...
                entries = soup.select("div.winner-item, article, div.project-item")
                for entry in entries:
                    try:
                        ...
                        author_elem = entry.select_one("span.designer, div.creator")
