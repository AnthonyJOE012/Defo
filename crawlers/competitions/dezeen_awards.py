...
class DezeenAwardsCrawler(BaseCrawler):
    """Crawler for Dezeen Awards (https://www.dezeenawards.com)."""

    BASE_URL = "https://www.dezeenawards.com"

    def __init__(self):
        super().__init__("Dezeen Awards", SourceType.COMPETITION)

    async def fetch_articles(self, **kwargs) -> list[Article]:
        """Fetch competition listings from Dezeen Awards."""
        articles = []
        headers = {
        ...
        async with httpx.AsyncClient(timeout=30.0, headers=headers) as client:
            try:
                ...
                entries = soup.select("div.project-item, article, div.winner-item")
                for entry in entries:
                    try:
                        title_elem = entry.select_one("h3, h2, h4.project-title")
                        ...
                        author_elem = entry.select_one("span.designer, div.architect")
    ...
    async def parse_article(self, url: str, **kwargs) -> Article | None:
        ...
        async with httpx.AsyncClient(timeout=30.0, headers=headers) as client:
            try:
                ...
                content_elem = soup.select_one("div.project-content, article")
