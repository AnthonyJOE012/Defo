"""ADC (Art Directors Club) competition crawler."""
...
class AdcCrawler(BaseCrawler):
    """Crawler for ADC (https://adc.global)."""

    BASE_URL = "https://adc.global"

    def __init__(self):
        super().__init__("ADC (Art Directors Club)", SourceType.COMPETITION)

    async def fetch_articles(self, **kwargs) -> list[Article]:
        """Fetch competition listings from ADC."""
        articles = []
        headers = {
        ...
        async with httpx.AsyncClient(timeout=30.0, headers=headers) as client:
            try:
                ...
                entries = soup.select("div.project-item, article.entry, div.adc-project")
