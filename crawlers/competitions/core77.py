...
class Core77Crawler(BaseCrawler):
    """Crawler for Core77 Design Awards (https://designawards.core77.com)."""

    BASE_URL = "https://designawards.core77.com"

    def __init__(self):
        super().__init__("Core77 Design Awards", SourceType.COMPETITION)

    async def fetch_articles(self, **kwargs) -> list[Article]:
        """Fetch competition listings from Core77."""
        articles = []
        headers = {
        ...
        async with httpx.AsyncClient(timeout=30.0, headers=headers) as client:
            try:
                ...
                entries = soup.select("div.entry, article, div.project-item")
                for entry in entries:
                    try:
                        title_elem = entry.select_one("h3, h2.title, h4")
                        ...
                        author_elem = entry.select_one("div.designer, span.byline")
    ...
    async def parse_article(self, url: str, **kwargs) -> Article | None:
        ...
        async with httpx.AsyncClient(timeout=30.0, headers=headers) as client:
            try:
                ...
                content_elem = soup.select_one("div.entry-content, article")
