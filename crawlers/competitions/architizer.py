...
class ArchitizerCrawler(BaseCrawler):
    """Crawler for Architizer Awards (https://architizer.com)."""

    BASE_URL = "https://architizer.com"

    def __init__(self):
        super().__init__("Architizer Awards", SourceType.COMPETITION)

    async def fetch_articles(self, **kwargs) -> list[Article]:
        """Fetch competition listings from Architizer."""
        articles = []
        headers = {
        ...
        async with httpx.AsyncClient(timeout=30.0, headers=headers) as client:
            try:
                ...
                entries = soup.select("div.project-item, article, div.entry")
                for entry in entries:
                    try:
                        ...
                        author_elem = entry.select_one("span.architect, div.firm")
    ...
    async def parse_article(self, url: str, **kwargs) -> Article | None:
        ...
        async with httpx.AsyncClient(timeout=30.0, headers=headers) as client:
            try:
                ...
                content_elem = soup.select_one("div.project-body, article")
