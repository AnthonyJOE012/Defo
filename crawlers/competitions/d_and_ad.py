...
class DAndADCrawler(BaseCrawler):
    """Crawler for D&AD (https://www.dandad.org)."""

    BASE_URL = "https://www.dandad.org"

    def __init__(self):
        super().__init__("D&AD", SourceType.COMPETITION)

    async def fetch_articles(self, **kwargs) -> list[Article]:
        """Fetch competition listings from D&AD."""
        articles = []
        headers = {
        ...
        async with httpx.AsyncClient(timeout=30.0, headers=headers) as client:
            try:
                response = await client.get(f"{self.BASE_URL}/awards")
                response.raise_for_status()
                soup = BeautifulSoup(response.text, "html.parser")

                entries = soup.select("div.nomination-item, article, div.winner")
                for entry in entries:
                    try:
                        title_elem = entry.select_one("h3, h2, div.title")
                        ...
                        author_elem = entry.select_one("div.creator, span.author")
