"""Tokyo Type Directors Club competition crawler."""
...
class TokyoTdcCrawler(BaseCrawler):
    """Crawler for Tokyo TDC (https://tdc.org)."""

    BASE_URL = "https://tdc.org"

    def __init__(self):
        super().__init__("Tokyo Type Directors Club", SourceType.COMPETITION)

    async def fetch_articles(self, **kwargs) -> list[Article]:
        """Fetch competition listings from Tokyo TDC."""
        articles = []
        headers = {
        ...
        async with httpx.AsyncClient(timeout=30.0, headers=headers) as client:
            try:
                ...
                entries = soup.select("div.project-item, article.entry, div.tdc-project")
