...
class DesignNewsCrawler(BaseCrawler):
    """Crawler for Design News articles."""

    BASE_URL = "https://www.designnews.com"

    def __init__(self):
        super().__init__(source_name="Design News", source_type=SourceType.NEWS)
    ...
    async def fetch_articles(self, **kwargs) -> list[Article]:
        """Fetch latest articles from Design News."""
        articles = []
        ...
        try:
            ...
            print(f"Error fetching Design News articles: {e}")
    ...
    async def parse_article(self, url: str, **kwargs) -> Article | None:
        """Parse a single Design News article."""
        try:
            response = await self.client.get(url)
            response.raise_for_status()
