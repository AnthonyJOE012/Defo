"""Web Designer Depot news crawler."""
...
class WebDesignerDepotCrawler(BaseCrawler):
    """Crawler for Web Designer Depot news articles."""

    BASE_URL = "https://www.webdesignerdepot.com"

    def __init__(self):
        super().__init__(source_name="Web Designer Depot", source_type=SourceType.NEWS)
    ...
    async def fetch_articles(self, **kwargs) -> list[Article]:
        """Fetch latest articles from Web Designer Depot."""
        articles = []
        ...
        try:
            ...
            print(f"Error fetching Web Designer Depot articles: {e}")
    ...
    async def parse_article(self, url: str, **kwargs) -> Article | None:
        """Parse a single Web Designer Depot article."""
        try:
            response = await self.client.get(url)
            response.raise_for_status()
