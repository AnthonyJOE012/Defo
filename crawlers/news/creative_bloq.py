...
class CreativeBloqCrawler(BaseCrawler):
    """Crawler for Creative Bloq news articles."""

    BASE_URL = "https://www.creativebloq.com"

    def __init__(self):
        super().__init__(source_name="Creative Bloq", source_type=SourceType.NEWS)
    ...
    async def fetch_articles(self, **kwargs) -> list[Article]:
        """Fetch latest articles from Creative Bloq."""
        articles = []
        ...
        try:
            ...
            article_links = soup.select("article a, .cb-post a")
            ...
            print(f"Error fetching Creative Bloq articles: {e}")
    ...
    async def parse_article(self, url: str, **kwargs) -> Article | None:
        """Parse a single Creative Bloq article."""
        try:
            response = await self.client.get(url)
            response.raise_for_status()
