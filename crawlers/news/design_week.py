...
class DesignWeekCrawler(BaseCrawler):
    """Crawler for Design Week news articles."""

    BASE_URL = "https://www.designweek.co.uk"

    def __init__(self):
        super().__init__(source_name="Design Week", source_type=SourceType.NEWS)
    ...
    async def fetch_articles(self, **kwargs) -> list[Article]:
        """Fetch latest articles from Design Week."""
        articles = []
        ...
        try:
            ...
            print(f"Error fetching Design Week articles: {e}")
    ...
    async def parse_article(self, url: str, **kwargs) -> Article | None:
        """Parse a single Design Week article."""
        try:
            response = await self.client.get(url)
            response.raise_for_status()
