...
class DailyDesignNewsCrawler(BaseCrawler):
    """Crawler for Daily Design News articles."""

    BASE_URL = "https://dailydesignnews.com"

    def __init__(self):
        super().__init__(source_name="Daily Design News", source_type=SourceType.NEWS)
    ...
    async def fetch_articles(self, **kwargs) -> list[Article]:
        """Fetch latest articles from Daily Design News."""
        articles = []
        ...
        try:
            ...
            print(f"Error fetching Daily Design News articles: {e}")
    ...
    async def parse_article(self, url: str, **kwargs) -> Article | None:
        """Parse a single Daily Design News article."""
        try:
            response = await self.client.get(url)
            response.raise_for_status()
