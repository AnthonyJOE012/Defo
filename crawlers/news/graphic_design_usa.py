"""Graphic Design USA news crawler."""
...
class GraphicDesignUSACrawler(BaseCrawler):
    ...
    async def fetch_articles(self, **kwargs) -> list[Article]:
        """Fetch latest articles from Graphic Design USA."""
        articles = []
        ...
        try:
            ...
            print(f"Error fetching Graphic Design USA articles: {e}")
    ...
    async def parse_article(self, url: str, **kwargs) -> Article | None:
        """Parse a single Graphic Design USA article."""
        try:
            response = await self.client.get(url)
            response.raise_for_status()
