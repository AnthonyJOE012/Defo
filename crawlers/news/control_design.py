...
class ControlDesignCrawler(BaseCrawler):
    """Crawler for Control Design news articles."""

    BASE_URL = "https://www.controldesign.com"

    def __init__(self):
        super().__init__(source_name="Control Design", source_type=SourceType.NEWS)
    ...
    async def fetch_articles(self, **kwargs) -> list[Article]:
        """Fetch latest articles from Control Design."""
        articles = []
        ...
        try:
            ...
            print(f"Error fetching Control Design articles: {e}")
    ...
    async def parse_article(self, url: str, **kwargs) -> Article | None:
        """Parse a single Control Design article."""
        try:
            response = await self.client.get(url)
            response.raise_for_status()
