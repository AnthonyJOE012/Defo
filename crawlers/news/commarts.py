...
class CommArtsCrawler(BaseCrawler):
    """Crawler for CommArts news articles."""

    BASE_URL = "https://www.commarts.com"

    def __init__(self):
        super().__init__(source_name="CommArts", source_type=SourceType.NEWS)
    ...
    async def fetch_articles(self, **kwargs) -> list[Article]:
        """Fetch latest articles from CommArts."""
        articles = []
        ...
        try:
            ...
            print(f"Error fetching CommArts articles: {e}")
    ...
    async def parse_article(self, url: str, **kwargs) -> Article | None:
        """Parse a single CommArts article."""
        try:
            response = await self.client.get(url)
            response.raise_for_status()
