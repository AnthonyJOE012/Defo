"""World Architecture News news crawler."""
...
class WorldArchitectureNewsCrawler(BaseCrawler):
    """Crawler for World Architecture News articles."""

    BASE_URL = "https://www.worldarchitecturenews.com"

    def __init__(self):
        super().__init__(source_name="World Architecture News", source_type=SourceType.NEWS)
    ...
    async def fetch_articles(self, **kwargs) -> list[Article]:
        """Fetch latest articles from World Architecture News."""
        articles = []
        ...
        try:
            ...
            article_links = soup.select("article a, .article a")
            ...
            print(f"Error fetching World Architecture News articles: {e}")
    ...
    async def parse_article(self, url: str, **kwargs) -> Article | None:
        """Parse a single World Architecture News article."""
        try:
            response = await self.client.get(url)
            response.raise_for_status()
