"""AIGA Eye on Design news crawler."""
...
class AigaEyeOnDesignCrawler(BaseCrawler):
    """Crawler for AIGA Eye on Design news articles."""

    BASE_URL = "https://eyeondesign.aiga.org"

    def __init__(self):
        super().__init__(source_name="AIGA Eye on Design", source_type=SourceType.NEWS)
    ...
    async def fetch_articles(self, **kwargs) -> list[Article]:
        """Fetch latest articles from AIGA Eye on Design."""
        articles = []
        ...
        try:
            ...
            print(f"Error fetching AIGA Eye on Design articles: {e}")
    ...
    async def parse_article(self, url: str, **kwargs) -> Article | None:
        """Parse a single AIGA Eye on Design article."""
        try:
            response = await self.client.get(url)
            response.raise_for_status()
