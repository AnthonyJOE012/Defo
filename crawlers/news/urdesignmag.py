"""URDesign Magazine news crawler."""
...
class URDesignMagCrawler(BaseCrawler):
    """Crawler for URDesign Magazine news articles."""

    BASE_URL = "https://www.urdesignmag.com"

    def __init__(self):
        super().__init__(source_name="URDesign Magazine", source_type=SourceType.NEWS)
    ...
    async def fetch_articles(self, **kwargs) -> list[Article]:
        """Fetch latest articles from URDesign Magazine."""
        articles = []
        ...
        try:
            ...
            print(f"Error fetching URDesign Magazine articles: {e}")
    ...
    async def parse_article(self, url: str, **kwargs) -> Article | None:
        """Parse a single URDesign Magazine article."""
        try:
            response = await self.client.get(url)
            response.raise_for_status()
