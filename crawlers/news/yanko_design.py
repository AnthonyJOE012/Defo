...
class YankoDesignCrawler(BaseCrawler):
    """Crawler for Yanko Design news articles."""

    BASE_URL = "https://www.yankodesign.com"

    def __init__(self):
        super().__init__(source_name="Yanko Design", source_type=SourceType.NEWS)
    ...
    async def fetch_articles(self, **kwargs) -> list[Article]:
        """Fetch latest articles from Yanko Design."""
        articles = []
        ...
        try:
            ...
            print(f"Error fetching Yanko Design articles: {e}")
    ...
    async def parse_article(self, url: str, **kwargs) -> Article | None:
        """Parse a single Yanko Design article."""
        try:
            response = await self.client.get(url)
            response.raise_for_status()
