...
class DesignWorldCrawler(BaseCrawler):
    """Crawler for Design World news articles."""

    BASE_URL = "https://www.designworldonline.com"

    def __init__(self):
        super().__init__(source_name="Design World", source_type=SourceType.NEWS)
    ...
    async def fetch_articles(self, **kwargs) -> list[Article]:
        """Fetch latest articles from Design World."""
        articles = []
        ...
        try:
            ...
            print(f"Error fetching Design World articles: {e}")
    ...
    async def parse_article(self, url: str, **kwargs) -> Article | None:
        """Parse a single Design World article."""
        try:
            response = await self.client.get(url)
            response.raise_for_status()
